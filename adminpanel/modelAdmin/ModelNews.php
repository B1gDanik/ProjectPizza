...
class ModelNews
{
    ...
    public static function getNewsActionList()
    {
        ...
        $rows = $database->getAll("SELECT * FROM news ORDER BY news.id ASC");
    ...
    {
        $database = new Database();
        $users = $database->getAll("SELECT user.* FROM user WHERE news.idUser = user.id");
    ...
    {
        $database = new Database();
        $row = $database->getOne("SELECT news.* FROM news WHERE news.id = $id");
