...
class ModelProduct
{
    ...
    public static function getProductActionList()
    {
        ...
        $rows = $database->getAll("SELECT product.*, category.category FROM product, category WHERE product.idCategory = category.id ORDER BY product.id ASC");
    ...
    {
        $database = new Database();
        $row = $database->getOne("SELECT product.*, category.category FROM product, category WHERE product.idCategory = category.id AND product.id = $id");
